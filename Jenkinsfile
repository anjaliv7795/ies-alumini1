...
finally
{
    ...
    stage('Create Docker Image for Major/Minor Release') {
        ...
        if (pkgJsonSemVer == "1.0.0")   //As goddamn git describe fails if no tags are present, another way of first-time build run test
        {
            ...
            {    
                ...
                oldNightlyBuildImage = sh(script:"docker images --filter=reference='ies-alumni:*NIGHTLY_BUILD*' -q | xargs --no-run-if-empty docker image rm", returnStdout: true)
                println "Value is: " + oldNightlyBuildImage
                nightlyBuildImage = docker.build("ies-alumni:${fullVersion}") //  Build another image & tag it for reference -also trying out diff image name
                    //sh "docker tag ies-alumni:latest ies-alumni:${fullVersion}" //\$\{gitTagVersion\}
            }
            else
            {
        ...
        else // Git tagging is done atleast once, so safely use git describe
        {
            ...
            if ((majorFromGitTag < majorFromPkgJson) || (minorFromGitTag < minorFromPkgJson))
            {
                ...
                releaseBuildImage = docker.build("ies-alumni:${fullVersion}")   //  Build another image & tag it for reference -also trying out diff image name
                //sh "docker tag ies-alumni:latest ies-alumni:${fullVersion}" //\$\{gitTagVersion\}
            }
            else
            {
    ...
    if (versionToFileAndTagtoGit == true) {   
        ...
        {
            sh "git remote set-url origin https://arunkrishpsg@bitbucket.org/saravananpsg/ies-alumni.git"    //still needed, search for workaround later
...
//Optionally, we can also add a test stage to test a Docker image using code like this:
    ...
    stage('Run image') {
       /*command to run the latest image */
        ...
        String containerExists = sh(script: "docker ps -f \"name=ies-alumni\" --format '{{.Names}}'", returnStdout: true)    //returns name of running container
        ...
        if (containerExists.trim().toString().equals("ies-alumni"))
        {
            sh "docker stop ies-alumni"
            sh "docker rm ies-alumni"
        }
        // Only latest image will be containerized - release or nightly build images must be manually containerized, if needed.
        sh "docker run -p 8015:8015 --name ies-alumni --restart=always --net=mongo-network ies-alumni:latest &"
        sh "sleep 5s"
        
        sh "docker restart ies-alumni"
    }
    ...
    stage('Restore DB dump') {
        ...
        sh "docker exec -i mongodbctr bash -c 'mongorestore --db ies-alumni --drop --gzip /data/ies-alumni/' "  
...
def notifyBuildStatus(String buildStatus) {
   ...
   details = """<p>Nightly Build for the Job '${env.JOB_NAME}' with Build No: [${env.BUILD_NUMBER}] finished with status '${buildStatus}'</p>
    ...
    emailext (
       ...
       subject: subject,
       body: details,
       to: 'saravanakumar.b@psgsoftwaretechnologies.com;cc:arun@psgsoftwaretechnologies.com;subramanian.nh@psgsoftwaretechnologies.com',
