...
/**
  ...
  @param {string} realmName - The name of the realm(not the realmID) - ex: master,
  @param {string} userId - The id of user
  @param {string} groupId - groupId to join
  @returns {Promise} A promise that resolves.
  @example
  keycloakAdminClient(settings)
    .then((client) => {
      client.users.joinGroup(realmName, userId, groupId)
        .then(() => {
          console.log('success')
      })
    })
...
function joinGroup (c) {
  return function joinGroup (realmName, userId, groupId) {
    return new Promise((resolve, reject) => {
      if (!userId) {
        return reject(new Error('userId is missing'));
      }
      ...
      if (!groupId) {
        return reject(new Error('Groupid is missing'));
      }

      const req = {
        url: `${c.baseUrl}/admin/realms/${realmName}/users/${userId}/groups/${groupId}`,
        auth: {
          bearer: privates.get(c).accessToken
        },
        json: true,
